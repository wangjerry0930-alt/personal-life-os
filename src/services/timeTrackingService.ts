...
export function saveTrackedTimeEntry(entry:TimeEntry){saveTimeEntry(entry);if(entry.category==='Entertainment'&&entry.entertainmentSubtype==='Gaming')consumeGamingMinutes(entry.durationMinutes,localDateKey(new Date(entry.endedAt)))}
