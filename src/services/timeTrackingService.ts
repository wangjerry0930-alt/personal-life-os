...
import { appendTimeEntry, loadSnapshot } from '../repositories/appRepository';
...
export function createManualTimeEntry(title:string,category:string,startedAt:string,endedAt:string,durationMinutes:number,notes=''):TimeEntry{return{id:`time-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,createdAt:endedAt,updatedAt:endedAt,title,category,startedAt,endedAt,durationMinutes,source:'manual',notes}}
