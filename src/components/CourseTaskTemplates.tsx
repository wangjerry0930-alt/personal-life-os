...
export default function CourseTaskTemplates({ onStartTimer }: { onStartTimer?: (task: { id: string; title: string; description?: string }) => void }) {
  ...
  const courseNames = Array.from(new Set(courseTasks.map(task => task.notes?.split(' · ')[1] || 'Course')));
  const visibleCourseTasks = courseTasks.filter(task => (courseFilter === 'All courses' || task.notes?.split(' · ')[1] === courseFilter) && (kindFilter === 'All types' || task.notes?.includes('· ' + kindFilter)));
