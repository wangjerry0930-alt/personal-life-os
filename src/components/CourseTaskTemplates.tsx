...
const kindFromNotes = (notes?: string) => notes?.match(/^Course:\s*.*?\s*·\s*([^·]+)/)?.[1]?.trim() || 'Task';
const planPrefix = (id: string) => id.replace(/-\d+$/, '');

export default function CourseTaskTemplates({ onStartTimer }: { onStartTimer?: (task: { id: string; title: string; description?: string }) => void }) {
  ...
  const courseNames = Array.from(new Set(courseTasks.map(task => courseFromNotes(task.notes)).filter(Boolean)));
  const visibleCourseTasks = courseTasks.filter(task => (courseFilter === 'All courses' || courseFromNotes(task.notes) === courseFilter) && (kindFilter === 'All types' || kindFromNotes(task.notes) === kindFilter));
  const removeTask = (id: string, title: string) => { if (!window.confirm(`Remove ${title}?`)) return; setData(current => ({ ...current, tasks: current.tasks.filter(task => task.id !== id) })); };
  const removePlan = (id: string) => { const prefix = planPrefix(id); const count = data.tasks.filter(task => planPrefix(task.id) === prefix).length; if (!window.confirm(`Remove this entire course plan (${count} task${count === 1 ? '' : 's'})?`)) return; setData(current => ({ ...current, tasks: current.tasks.filter(task => planPrefix(task.id) !== prefix) })); };
  ...
  return <>
    <section className="uni-course-panel"><div className="section-title"><div><span className="pill purple">UNI COURSE</span><h3>University course plan</h3><span className="muted">{courseTasks.filter(task => task.status !== 'done').length} open · {courseTasks.length} total</span></div><div className="uni-course-actions"><select className="uni-course-filter" value={courseFilter} onChange={event => setCourseFilter(event.target.value)}><option>All courses</option>{courseNames.map(name => <option key={name}>{name}</option>)}</select><select className="uni-course-filter" value={kindFilter} onChange={event => setKindFilter(event.target.value)}><option>All types</option>{Object.keys(templates).map(item => <option key={item}>{item}</option>)}</select><button className="secondary" onClick={() => setOpen(true)}><Icon name="Plus" size={15}/> Add course plan</button><button className="icon-button" onClick={() => setHubOpen(value => !value)} aria-label={hubOpen ? 'Collapse university course plan' : 'Expand university course plan'}>{hubOpen ? '−' : '+'}</button></div></div>{hubOpen && <>{visibleCourseTasks.length ? <div className="uni-course-task-list">{visibleCourseTasks.map(task => <div className={'uni-course-task '+(task.status === 'done' ? 'done' : '')} key={task.id}><button className="check" onClick={() => toggleTask(task.id)}>{task.status === 'done' ? '✓' : '○'}</button><div><small className="course-task-kind">{kindFromNotes(task.notes)}</small><b>{task.title}</b><small>{task.scheduledDate || 'Unscheduled'} · {task.minutes} min · {task.description}</small></div>{task.status !== 'done' && onStartTimer && <button className="secondary course-task-start" onClick={() => onStartTimer(task)}>Start timer</button>}<div className="course-task-remove"><button onClick={() => removeTask(task.id, task.title)}>Remove task</button><button onClick={() => removePlan(task.id)}>Remove plan</button></div><span>{task.status === 'done' ? 'Done' : 'Planned'}</span></div>)}</div> : <p className="muted">No university course plans yet. Add a preview, study, review, assignment or exam task.</p>}</>}
    </section>
