...
export default function AnalyticsPage(){
  ...
  return <div className="content">
    ...
    <section className="panel analytics-review"><div className="section-title"><h3>{range===7?'Weekly':'Monthly'} review</h3><span className="pill teal">AUTO-GENERATED</span></div><p>You completed <b>{completed}</b> tasks and invested <b>{minutes}</b> minutes across the last {range} days. {habitLogs.length?`Your recorded habit completion was ${habitRate}%.`:'There is not enough habit history for a completion rate yet.'}</p><p className="muted">{minutes===0?'Start with one small timed session today.':average<20?'Keep the rhythm with a short daily focus block.':'Your consistency is building—protect the sessions that create the most progress.'} {topSkill?`Next, continue practicing ${topSkill}.`:topProject?`Your most active project was ${topProject}.`:'Choose one focus area for the next session.'}</p></section>
