...
export default function FocusTimer() {
  ...
  return <><MorningChecklist/><DailyPlanPanel/><section className="focus-timer"><div><span className="pill teal">FOCUS TIMER</span><h3>{minutes}:{secs}</h3><p>{seconds===0?'Session complete. Nice work.':'A calm 25-minute block for one small step.'}</p></div><div className="focus-timer-actions"><button className="primary" onClick={()=>setRunning(!running)}><Icon name={running?'Pause':'Play'} size={15}/>{running?'Pause':'Start'}</button><button className="secondary" onClick={()=>{setRunning(false);setSeconds(25*60)}}><Icon name="RefreshCw" size={15}/> Reset</button></div></section></>;
}
