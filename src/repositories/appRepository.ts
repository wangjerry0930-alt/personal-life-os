...
export function appendTimeEntry(entry:TimeEntry){const snapshot=loadSnapshot();const activity:ActivityEntity={id:`activity-time-${entry.id}`,createdAt:entry.createdAt,updatedAt:entry.updatedAt,type:'LearningSession',title:entry.title,description:`${entry.category} · ${entry.source}`,occurredAt:entry.endedAt,durationMinutes:entry.durationMinutes,source:'Time Tracking',metadata:{category:entry.category,taskId:entry.taskId,projectId:entry.projectId,areaId:entry.areaId,skillId:entry.skillId}};saveSnapshot({...snapshot,timeEntries:[entry,...snapshot.timeEntries].slice(0,5000),activities:[activity,...snapshot.activities].slice(0,1000)});}
