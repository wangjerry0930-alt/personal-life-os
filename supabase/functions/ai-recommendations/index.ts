  ...
  try {
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        ...(isBookAnalysis ? { text: { format: { type: 'json_schema', name: 'book_extraction', strict: true, schema: { type: 'object', additionalProperties: false, properties: { bookType: { type: 'string' }, coreThesis: { type: 'string' }, concepts: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { name: { type: 'string' }, summary: { type: 'string' }, tier: { type: 'integer', enum: [1,2,3,4] }, whyEssential: { type: 'string' }, prerequisites: { type: 'array', items: { type: 'string' } }, sourceChunkIds: { type: 'array', items: { type: 'string' } }, centralityScore: { type: 'number' }, dependencyScore: { type: 'number' }, bookThesisImportance: { type: 'number' }, userRelevanceScore: { type: 'number' }, practicalValue: { type: 'number' }, redundancyPenalty: { type: 'number' } }, required: ['name','summary','tier','whyEssential','prerequisites','sourceChunkIds','centralityScore','dependencyScore','bookThesisImportance','userRelevanceScore','practicalValue','redundancyPenalty'] } }, recalls: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { question: { type: 'string' }, answer: { type: 'string' }, difficulty: { type: 'string', enum: ['Easy','Medium','Hard'] }, conceptName: { type: 'string' } }, required: ['question','answer','difficulty','conceptName'] } } }, required: ['bookType','coreThesis','concepts','recalls'] } } } : isBookAnswer ? { text: { format: { type: 'json_schema', name: 'book_answer', strict: true, schema: { type: 'object', additionalProperties: false, properties: { answer: { type: 'string' }, confidence: { type: 'number' }, insufficientEvidence: { type: 'boolean' }, citations: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { chunkId: { type: 'string' }, pageStart: { type: 'integer' }, pageEnd: { type: 'integer' }, supportingQuote: { type: 'string' } }, required: ['chunkId','pageStart','pageEnd','supportingQuote'] } }, followUpQuestions: { type: 'array', items: { type: 'string' } } }, required: ['answer','confidence','insufficientEvidence','citations','followUpQuestions'] } } } } : {}),
      }),
    });
